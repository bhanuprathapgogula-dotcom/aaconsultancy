  ...
  return (
    <section id="portfolio" className="relative py-24 bg-bg-dark overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12 max-w-4xl mx-auto p-1 bg-white/5 border border-white/10 rounded-2xl md:rounded-full backdrop-blur-md">
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                className="group glass-panel rounded-2xl overflow-hidden border border-white/5 bg-gradient-to-b from-white/3 to-transparent flex flex-col justify-between h-[420px] relative shadow-lg shadow-black/20"
              >
                {/* Media Container */}
                <div className="h-[230px] w-full relative overflow-hidden bg-zinc-950 border-b border-white/5 flex items-center justify-center">
                  {project.isCustomSvg ? (
                    project.svgMarkup
                  ) : (
                    <Image
                  ...
                  {/* Dark Glass Overlay */}
                  <div className="absolute inset-0 bg-bg-dark/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4 z-10 backdrop-blur-[2px]">
                    <button
                      onClick={() => handleScroll("#contact")}
                      className="p-3 rounded-full bg-neon-blue text-white shadow-lg shadow-neon-blue/30 hover:scale-110 transition-transform duration-300"
                    >
                    ...
                    <a
                      ...
                      className="p-3 rounded-full bg-white/10 text-white border border-white/20 hover:bg-white/20 hover:scale-110 transition-all duration-300"
                    >
                ...
                {/* Info Container */}
                <div className="p-6 flex flex-col flex-1 justify-between relative z-20 bg-bg-dark/30 backdrop-blur-sm">
                  <div>
                    {/* Category tag */}
