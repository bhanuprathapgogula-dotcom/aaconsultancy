        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <motion.div
              ...
              className={`group glass-panel rounded-2xl p-6 md:p-8 border border-glass-border bg-gradient-to-tr from-glass-bg to-transparent ${feature.glowClass} transition-all duration-500 hover:-translate-y-1.5 flex flex-col justify-between`}
            >
              <div>
                {/* Feature Icon Header */}
                <div className="mb-6 inline-flex p-3 rounded-xl bg-glass-bg border border-glass-border group-hover:bg-glass-hover group-hover:border-glass-border transition-all duration-300">
                  {feature.icon}
                </div>
                ...
                <p className="text-xs md:text-sm text-muted font-light leading-relaxed group-hover:text-muted transition-colors">
                  {feature.desc}
                </p>
              </div>

              {/* Glowing decorative indicator */}
              <div className="mt-8 h-1 w-8 rounded-full bg-glass-hover group-hover:w-16 group-hover:bg-gradient-to-r group-hover:from-neon-blue group-hover:to-neon-cyan transition-all duration-500" />
            </motion.div>
          ))}
        </div>
      </div>
