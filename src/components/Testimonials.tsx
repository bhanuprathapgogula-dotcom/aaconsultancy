    ...
    <section className="relative py-24 bg-transparent overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="relative max-w-4xl mx-auto flex items-center justify-center min-h-[380px]">
          ...
          <div className="w-full relative z-10">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="glass-panel rounded-3xl p-8 md:p-12 border border-glass-border bg-gradient-to-br from-glass-bg to-transparent relative shadow-2xl shadow-black/30 flex flex-col md:flex-row gap-8 items-start"
              >
                {/* Large Quote graphic mark */}
                <Quote className="absolute top-6 right-8 w-16 h-16 text-foreground/5 stroke-[1.5]" />
                ...
                <div className="flex flex-col items-center md:items-start shrink-0 text-center md:text-left">
                  <div className={`h-20 w-20 rounded-2xl bg-gradient-to-br ${testimonials[activeIndex].avatarColor} p-[2px] shadow-lg shadow-neon-blue/10 mb-4`}>
                    <div className="h-full w-full rounded-[22px] bg-background flex items-center justify-center">
                      <span className="text-2xl font-black text-foreground tracking-wider">
                ...
                <div className="flex-1 flex flex-col justify-between h-full pt-2">
                  <div>
                    ...
                    {/* Review text */}
                    <p className="text-sm md:text-base text-muted font-light leading-relaxed mb-6">
                    ...
                    {testimonials.map((_, idx) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all duration-300 ${
                          activeIndex === idx ? "w-6 bg-neon-cyan" : "w-1.5 bg-glass-hover hover:bg-glass-hover"
                        }`}
                      />
                    ))}
                  </div>
          ...
          <button
            onClick={handlePrev}
            className="absolute left-[-20px] md:left-[-70px] z-20 p-3 rounded-full bg-glass-bg border border-glass-border hover:bg-glass-hover hover:border-glass-border transition-all text-muted hover:text-foreground"
          >
          ...
          <button
            onClick={handleNext}
            className="absolute right-[-20px] md:right-[-70px] z-20 p-3 rounded-full bg-glass-bg border border-glass-border hover:bg-glass-hover hover:border-glass-border transition-all text-muted hover:text-foreground"
          >
