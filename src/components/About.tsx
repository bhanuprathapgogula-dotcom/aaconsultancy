      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-20">
          ...
          <div className="lg:col-span-6 flex flex-col">
            {/* Tab buttons */}
            <div className="flex gap-2 p-1.5 rounded-full bg-white/5 border border-white/10 mb-8 self-start backdrop-blur-sm">
            ...
            {/* Tab content panel */}
            <div className="glass-panel rounded-2xl p-8 md:p-10 border border-white/5 relative min-h-[340px] flex flex-col justify-between shadow-xl shadow-black/20">
                  ...
                  <ul className="space-y-3.5 mt-auto">
                    {tabContents[activeTab as keyof typeof tabContents].bullets.map((bullet, index) => (
                      <li key={index} className="flex items-center gap-3">
                        ...
                        <span className="text-xs md:text-sm text-gray-300 font-light">{bullet}</span>
                      </li>
                    ))}
                  </ul>
                </motion.div>
          ...
          <div className="lg:col-span-6 relative flex flex-col h-full justify-center">
            <div className="relative glass-panel rounded-2xl p-8 border border-white/5 flex flex-col justify-center overflow-hidden bg-gradient-to-br from-white/3 to-white/0 shadow-xl shadow-black/25">
              ...
              <blockquote className="text-base md:text-lg text-gray-300 italic font-light leading-relaxed mb-6">
              ...
              <div className="flex items-center gap-4">
                <div className="h-10 w-10 rounded-full bg-gradient-to-tr from-neon-blue to-neon-purple p-[1px]">
                  <div className="h-full w-full rounded-full bg-bg-dark flex items-center justify-center text-xs font-bold text-white">
                    AA
                  </div>
                </div>
                <div>
                  <span className="block text-sm font-bold text-white leading-none">Engineering Directorate</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {statItems.map((stat, index) => (
            <motion.div
              ...
              className="glass-panel glass-panel-hover rounded-2xl p-6 border border-white/5 flex flex-col items-start shadow-lg shadow-black/15 group relative"
            >
              {/* Top Row: Icon */}
              <div className="mb-4 p-2.5 rounded-xl bg-white/5 border border-white/10 group-hover:bg-white/10 transition-colors">
                {stat.icon}
              </div>

              {/* Stat Value */}
              <span className="text-3xl font-extrabold tracking-tight text-white mb-1 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent group-hover:from-neon-blue group-hover:to-neon-cyan group-hover:text-glow-blue transition-all duration-500 font-mono">
                {stat.value}
              </span>

              {/* Label */}
              <span className="text-xs font-bold text-white uppercase tracking-wider mb-0.5">
                {stat.label}
              </span>
